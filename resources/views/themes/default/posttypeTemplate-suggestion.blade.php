    ...
    <section class="uk-section">
        <div class="uk-container">
            <ul uk-tab class="uk-why-us-tab">
                @foreach ($posts->where('id', '!=', 163) as $row)
                    <li><a href="">{{$row->post_title}}</a></li>
                @endforeach
            </ul>
            <div class="uk-switcher uk-margin uk-margin-top">
                @foreach ($posts->where('id', '!=', 163) as $value)
                    <div>
                        <div>
...
@php
    $post = $posts->firstWhere('id', 163);
@endphp

@if ($post)
    <section class=" uk-primary-bg uk-about-text">
        <div>
            <div class="uk-grid-collapse uk-grid" uk-height-match="target: .uk-same-height">
                <div class="uk-width-2-3@m uk-same-height">
                    <div class="uk-contents uk-title-font uk-container uk-section uk-padding-large">
                        <h1 class="uk-secondary">{{$post->post_title}}</h1>
                ...
                <div class="uk-width-1-3@m">
                    <img src="{{ $post->page_thumbnail ?asset('uploads/medium/'.$post->page_thumbnail) : asset('theme-assets/img/mountain/mountain8.jpeg')}}" class="cover uk-same-height" alt="{{$post->post_title}}">
                </div>
            </div>
        </div>
    </section>
