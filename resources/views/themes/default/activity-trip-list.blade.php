...
<section class="bg-black-light  uk-hero-banner uk-cover-container uk-position-relative uk-flex uk-flex-bottom 
   uk-background-norepeat uk-background-cover uk-background-top-center uk-background-fixed" uk-height-viewport data-src="{{asset('uploads/banners/'.$data->banner)}} " uk-img>
    ...
    <div class="uk-position-center uk-text-center uk-inner-banner" uk-scrollspy="cls: uk-animation-slide-left-small; target:  h5,p;  delay: 50; repeat: false;">
        ...
        <p class="banner-heading  uk-margin-remove">{{ $data->title }}</p>
...
<section class="uk-section">
    <div class="uk-container">
        <ul class="uk-child-width-1-1  uk-child-width-1-2@m  uk-child-width-1-3@l uk-flex uk-flex-center uk-grid" uk-grid uk-height-match="target:.uk-trek-list">
            <!--trek -->
        @if($trips != null)
        @foreach($trips as $item)
            <li>
                ...
                @endif
                        <div class="uk-position-top-right uk-overlay uk-overlay-default">{{ $data->title }}</div>
                    </div>
